...
normalizeBurnRate <- function(calFrame, dataFrame, filterCol, filter, panelType='RP') {
  ...
  start.group <- min(as.character(aggFrame[!(is.na(aggFrame$YearWeek)), 'YearWeek']))
  aggFrame.fill <- aggFrame.fill[as.character(aggFrame.fill$YearWeek) >= start.group, ]
  ...
  if(panelType!='RP') {
    ...
    gap.finder <- data.frame(Index = seq(1, length(aggFrame.fill$RunRate), 1))
    holes <- which(aggFrame.fill$RunRate <= 1)  
  ...
  if(length(holes) > 0) {
    ...
    if(panelType!='RP') {
      ...
      holes <- data.frame(Holes = which(aggFrame.fill$RunRate <= 1), Count = 1)
    } else {
  ...
  if(length(aggFrame.good[,1]) < 28) { return() } # there needs to be at least 28 weeks of good data so there can be half a year of good roll data (26 weeks)
