...
p.codes.torch.1 <- ggplot(subset(codes.lim, Version == 'Torch'), aes(x=DateGroup, y=Rate, group=Category, color=Color)) + geom_line(color='black') + geom_point() + scale_color_manual(values=c('black','black'), guide=FALSE) + geom_hline(aes(yintercept=UL), color='blue', lty='dashed') + facet_wrap(~Code, scale='free_y') + scale_y_continuous(labels=percent) + labs(title='Code Usage Rate per RMAs Shipped - Torch:\nFYI Limit = +2 standard deviations',x='Date\n(Year-Week)', y='4-week Rolling Average Rate') + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90, hjust=1)) + scale_x_discrete(breaks = dateBreaks) 
...
# p.failures.version <- ggplot(failures.rate.verison, aes(x=DateGroup, y=Rate, fill=Version)) + geom_bar(stat='identity') + facet_wrap(~Version+Department) + scale_fill_manual(values=pal.fail.version) + scale_y_continuous(labels=percent) + labs(title='Early Failures per Instruments Shipped:\nGoal = 3.5% of Instruments Shipped',x='Date\n(Year-Week)', y='4-week Rolling Average Rate') + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90, hjust=1)) + scale_x_discrete(breaks = dateBreaks) + geom_hline(aes(yintercept=0.035), color='black', lty=2)
# p.failures.version <- ggplot(failures.rate.verison, aes(x=DateGroup, y=Rate, fill=Version)) + geom_bar(stat='identity', position='dodge') + facet_wrap(~Department, ncol=1) + scale_fill_manual(values=pal.fail.version) + scale_y_continuous(labels=percent) + labs(title='Early Failures per Instruments Shipped:\nGoal = 3.5% of Instruments Shipped',x='Date\n(Year-Week)', y='4-week Rolling Average Rate') + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90, hjust=1)) + scale_x_discrete(breaks = dateBreaks) + geom_hline(aes(yintercept=0.035), color='black', lty=2)
...
prod.fail.month <- prod.fail.month[!(prod.fail.month$Version == 'Torch' & prod.fail.month$DateGroup < '2016-09'), ]
...
p.prod.fail.month <- ggplot(prod.fail.month, aes(x=DateGroup, y=Rate, fill=Key)) + geom_bar(stat='identity') + scale_fill_manual(values=createPaletteOfVariableLength(prod.fail.month, 'Key'), name='') + facet_wrap(~Version, ncol=1, scale='free_y') + geom_line(data = prod.fail.month, aes(x=DateGroup, y=CumulativeRate, group=Key, color=Key), lwd=1.5, lty='dashed') + scale_color_manual(values=c('black','black'), guide=FALSE) + scale_y_continuous(label=percent) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(angle=90, hjust=1)) + labs(title='Early Failure Rates by Month\n(12 Month Cumulative Rate Overlay)', x='Date\n(Year-Month)', y='Failures/New Instruments Shipped')
p.serv.fail.month <- ggplot(serv.fail.month, aes(x=DateGroup, y=Rate, fill=Key)) + geom_bar(stat='identity') + scale_fill_manual(values=createPaletteOfVariableLength(serv.fail.month, 'Key'), name='') + facet_wrap(~Version, ncol=1, scale='free_y') + geom_line(data = serv.fail.month, aes(x=DateGroup, y=CumulativeRate, group=Key, color=Key), lwd=1.5, lty='dashed') + scale_color_manual(values=c('black','black'), guide=FALSE) + scale_y_continuous(label=percent) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(angle=90, hjust=1)) + labs(title='Early Failure Rates by Month\n(12 Month Cumulative Rate Overlay)', x='Date\n(Year-Month)', y='Failures/RMA Instruments Shipped')
