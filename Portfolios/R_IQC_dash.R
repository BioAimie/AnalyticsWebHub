...
p.run.review <- ggplot(subset(run.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version, scale='free_y') + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent, limits=c(0,1)) + scale_x_discrete(breaks=dateBreaks) + geom_hline(aes(yintercept=UL), color='red', lty=2) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='Pouch Review Rates:\nLimit = mean + 3sdev   (Torch limits not actionable)')
...
y_positions <- 0.10
p.rna.review <- ggplot(subset(rna.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version, scale='free_y') + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + geom_hline(aes(yintercept=UL), color='red', lty=2) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='Yeast Control Review Rates:\nLimit = mean + 3sdev   (Torch limits not actionable)') + annotate(geom='text', x=x_positions, y=y_positions, label=annotations)
...
p.mp.tm.review <- ggplot(subset(mp.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version, scale='free_y') + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + geom_hline(aes(yintercept=UL), color='red', lty=2) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='60D Melt Probe Tm Range Review Rates:\nLimit = mean + 3sdev   (Torch limits not actionable)')
...
y_positions <- 0.12
p.mp.df.review <- ggplot(subset(mp.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version) + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + geom_hline(aes(yintercept=UL), color='red', lty=2) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='60D Melt Probe Median dF Review Rates:\nLimit = mean + 3sdev   (Torch limits not actionable)')
...
p.noise.review <- ggplot(subset(noise.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version, scale='free_y') + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + geom_hline(aes(yintercept=UL), color='red', lty=2) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='Noise Review Rates:\nLimit = mean + 3sdev')
...
p.errormessages <- ggplot(errors.rate, aes(x=DateGroup, y=Rate, fill=RecordedValue, label=Annotation)) + geom_bar(stat='identity', aes(label=Annotation), data=errors.rate) + geom_line(data=errors.all.rate, inherit.aes=FALSE, aes(x=DateGroup, y=OverallRate, group = 1)) + geom_point(data=errors.all.rate, inherit.aes=FALSE, aes(x=DateGroup, y=OverallRate)) + geom_text() + facet_grid(Key~Version, scales='free_y') + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace), legend.position='bottom') + scale_y_continuous(label=percent) + labs(x='Date', y='Errors/Number of Runs', title='Instrument Error Rate in Final QC by Type') + scale_fill_manual(values = myPal, name='') + guides(fill=guide_legend(nrow=3, byrow=TRUE)) + geom_text(aes(x=DateGroup, y=0.1, label=Annotation), data=errors.rate)
p.errormessages.count <- ggplot(errors.num, aes(x=DateGroup, y=RunError, fill=RecordedValue)) + geom_bar(stat='identity') + facet_grid(Key~Version, scales='free_y') + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace), legend.position='bottom') + labs(x='Date', y='Errors', title='Instrument Error Count in Final QC by Type') + scale_fill_manual(values = myPal, name='') + guides(fill=guide_legend(nrow=3, byrow=TRUE))

# add a first-pass yield chart
firstPass.runs <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', firstPass.df, 'Key', startDate, 'Record', 'sum', 0)
firstPass.pass <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, Result=='Pass'), 'Key', startDate, 'Record', 'sum', 0)
firstPass.rate <- mergeCalSparseFrames(firstPass.pass, firstPass.runs, c('DateGroup','Key'), c('DateGroup','Key'), 'Record', 'Record', NA, 4)
p.first.pass <- ggplot(firstPass.rate, aes(x=DateGroup, y=Rate, group=Key)) + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='First Pass Yield (4-week moving average)', title='Instrument First Pass Yield in Final QC') + facet_wrap(~Key, ncol=1) + geom_hline(aes(yintercept=0.9), color='mediumseagreen', lty='dashed') + scale_y_continuous(labels=percent) + expand_limits(y=0.5)
