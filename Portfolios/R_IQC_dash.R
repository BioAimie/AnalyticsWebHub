...
# y_positions <- 0.10
p.rna.review <- ggplot(subset(rna.review.lims,Key != 'PouchQC'), aes(x=DateGroup, y=Rate, color=Color, group=1)) + geom_line(color='black') + geom_point() + facet_grid(Key~Version, scale='free_y') + scale_color_manual(values=c('blue','red'), guide=FALSE) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + geom_line(aes(y=UL), color='red', lty=2, group=1) + theme(plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date', y='4 Week Rolling Average', title='Yeast Control Review Rates:\nLimit = mean + 3sdev   (Torch limits not actionable)') # + annotate(geom='text', x=x_positions, y=y_positions, label=annotations)
...
errors.num <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', errors.num, c('Version','Key','RecordedValue'), plot.startDate.week, 'RunError', 'sum', 0)
errors.denom <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', errors.denom, c('Version','Key'), plot.startDate.week, 'RunError', 'sum', 1)
...
firstPass.runs <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, TestNo==1), 'Key', plot.startDate.week, 'Record', 'sum', 0)
firstPass.pass <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, TestNo==1 & Result=='Pass'), 'Key', plot.startDate.week, 'Record', 'sum', 0)
firstPass.rate <- mergeCalSparseFrames(firstPass.pass, firstPass.runs, c('DateGroup','Key'), c('DateGroup','Key'), 'Record', 'Record', NA, 4)
p.production.firstpass <- ggplot(subset(firstPass.rate, Key == 'Production'), aes(x=DateGroup, y=Rate, group=Key)) + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='First Pass Yield (4-week moving average)', title='Instrument First Pass Yield in Final QC', subtitle = 'Production') + geom_line(aes(y=0.9), color='mediumseagreen', lty='dashed', group=1) + scale_y_continuous(labels=percent) + expand_limits(y=0.5)
p.service.firstpass <- ggplot(subset(firstPass.rate, Key == 'Service'), aes(x=DateGroup, y=Rate, group=Key)) + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='First Pass Yield (4-week moving average)', title='Instrument First Pass Yield in Final QC', subtitle = 'Service') + geom_line(aes(y=0.9), color='mediumseagreen', lty='dashed', group=1) + scale_y_continuous(labels=percent) + expand_limits(y=0.5)
firstPass.fail <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, Result!='Pass'), c('Key','Result'), plot.startDate.week, 'Record', 'sum', 0)
...
firstPass.pouchremoved <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, TestNo == 1 & !grepl('Pouch-',as.character(firstPass.df[,'Result']))), 'Key', plot.startDate.week, 'Record', 'sum',0)
firstPass.ratepr <- mergeCalSparseFrames(firstPass.pass, firstPass.pouchremoved, c('DateGroup','Key'), c('DateGroup','Key'), 'Record', 'Record', NA, 4)
p.firstpass.nonpouch <- ggplot(firstPass.ratepr, aes(x=DateGroup, y=Rate, group=Key)) + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + facet_wrap(~Key, ncol=1) + theme(plot.subtitle = element_text(hjust=0.5), plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='First Pass Yield (4-week moving average)', title='Instrument First Pass Yield in Final QC', subtitle = 'Pouch-Related Failures Removed') + geom_line(aes(y=0.95), color='mediumseagreen', lty='dashed', group=1) + scale_y_continuous(labels=percent) + expand_limits(y=0.75)

# denominator charts
# IQC runs by Service, Production and Version??
runs.fill.trunc <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', runs.all, c('Key','Version'), plot.startDate.week, 'Record', 'sum', NA)
p.denom.runs <- ggplot(subset(runs.fill.trunc, Key!='PouchQC'), aes(x=DateGroup, y=Record, fill=Version)) + geom_bar(stat='identity') + facet_wrap(~Key, ncol=1) + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='Count of Runs in Final QC',title='Count of Runs Performed in QC by Week') + scale_fill_manual(values=createPaletteOfVariableLength(subset(runs.fill, Key!='PouchQC'), 'Version'))
firstPass.runs.trunc <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, TestNo==1), 'Key', plot.startDate.week, 'Record', 'sum', 0)
p.denom.first <- ggplot(firstPass.runs.trunc, aes(x=DateGroup, y=Record, fill='Fill')) + geom_bar(stat='identity') + facet_wrap(~Key, ncol=1) + scale_x_discrete(breaks=dateBreaks) + theme(plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='Count of Distinct Instruments to Final QC',title='Count of Distinct Instruments going to QC by Week') + scale_fill_manual(values=createPaletteOfVariableLength(data.frame(Fill='Fill'),'Fill'), guide=FALSE)
