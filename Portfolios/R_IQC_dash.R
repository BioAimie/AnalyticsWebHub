...
firstPass.pouchremoved <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(firstPass.df, TestNo == 1 & !grepl('Pouch-',as.character(firstPass.df[,'Result']))), 'Key', startDate, 'Record', 'sum',0)
...
p.firstpass.nonpouch <- ggplot(firstPass.ratepr, aes(x=DateGroup, y=Rate, group=Key)) + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + facet_wrap(~Key, ncol=1) + theme(plot.subtitle = element_text(hjust=0.5), plot.title=element_text(hjust=0.5, size=fontSize, face=fontFace), text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, face=fontFace), axis.text=element_text(size=fontSize, color='black', face=fontFace)) + labs(x='Date\n(Year-Week)', y='First Pass Yield (4-week moving average)', title='Instrument First Pass Yield in Final QC', subtitle = 'Pouch-Related Failures Removed') + geom_hline(aes(yintercept=0.95), color='mediumseagreen', lty='dashed') + scale_y_continuous(labels=percent) + expand_limits(y=0.75)
