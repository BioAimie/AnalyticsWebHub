...
start.month5 <- findStartDate(calendar.month, 'Month', 12, 5)
...
fontFace <- 'bold'
seqBreak <- 12
dateBreaks.wk <- as.character(unique(calendar.week[calendar.week[,'DateGroup'] >= start.week,'DateGroup']))[order(as.character(unique(calendar.week[calendar.week[,'DateGroup'] >= start.week,'DateGroup'])))][seq(1,length(as.character(unique(calendar.week[calendar.week[,'DateGroup'] >= start.week,'DateGroup']))), seqBreak)]
...
p.FilmArrayProduction <- ggplot(subset(instreleased, DateGroup >= beg), aes(x=DateGroup, y=Record, fill=Version)) + geom_bar(stat='identity') + geom_line(data=subset(goals.FAprod, as.character(DateGroup) <= current), inherit.aes=FALSE, aes(x=DateGroup, y=Goal, group=1), color='forestgreen', size=1) + geom_point(data=subset(goals.FAprod, as.character(DateGroup) <= current), inherit.aes=FALSE, aes(x=DateGroup, y=Goal, group=1), color='forestgreen') + labs(title = '2017 FilmArray Production', subtitle='Goal = Green Line', x = 'Date\n(Year-Month)', y = 'Instruments') + theme(text=element_text(size=fontSize, face=fontFace), axis.text.x=element_text(angle=90, vjust=0.5,color='black', size=fontSize), axis.text.y=element_text(hjust=1, color='black', size=fontSize), plot.title = element_text(hjust=0.5), plot.subtitle = element_text(hjust=0.5)) + scale_fill_manual(values=createPaletteOfVariableLength(instreleased, 'Version'))
...
newShip.month <- aggregateAndFillDateGroupGaps(calendar.month, 'Month', newShipments, c('Product'), start.monthyr, 'Record', 'sum', 0)
failures.month <- aggregateAndFillDateGroupGaps(calendar.month, 'Month', subset(fail.modes.df, Version != 'Torch Base'), c('Version','Fail','Department'), start.monthyr, 'Record', 'sum', 0)
...
firstPass.runs <- aggregateAndFillDateGroupGaps(calendar.week, 'Week', subset(firstPass.df, TestNo==1), 'Key', start.weekRoll, 'Record', 'sum', 0)
firstPass.pass <- aggregateAndFillDateGroupGaps(calendar.week, 'Week', subset(firstPass.df, TestNo==1 & Result=='Pass'), 'Key', start.weekRoll, 'Record', 'sum', 0)
...
wpfs.count <- wpfs.count[wpfs.count[,'DateGroup'] >= findStartDate(calendar.week, 'Week', 8), ]
...
# rate calculated by using 5 month moving average of instruments shipped as denominator
fails <- aggregateAndFillDateGroupGaps(calendar.month, 'Month', subset(fail.modes.df, Department == 'Production'), c('Fail', 'Version'), start.monthyr, 'DistinctRecord', 'sum', 0)
new.ship <- aggregateAndFillDateGroupGaps(calendar.month, 'Month', newShipments, 'Product', start.month5, 'Record', 'sum', 0)
#5 month moving average for denom for each version
new.shipavg <- c()
...
for(i in 1:length(versions)) {
  ...
  temp <- cbind(temp[5:l,], sapply(5:l, function(x) mean(temp[(x-4):x,'Record'])))
...
transferred.denom <- aggregateAndFillDateGroupGaps(calendar.month, 'Month', transferred.df, 'Key', start.month, 'Record', 'sum', 0)
...
ef.report.lead.fill <- aggregateAndFillDateGroupGaps(calendar.week, 'Week', leadingEF.df, c('Key','RecordedValue'), start.week, 'Record', 'sum', 0)
