...
dateBreaks.3yr <- as.character(unique(calendar.df[calendar.df[,'DateGroup'] >= threeyr.plot,'DateGroup']))[order(as.character(unique(calendar.df[calendar.df[,'DateGroup'] >= threeyr.plot,'DateGroup'])))][seq(4,length(as.character(unique(calendar.df[calendar.df[,'DateGroup'] >= threeyr.plot,'DateGroup']))), seqBreak)]
fontSize <- 20
fontFace <- 'bold'
...
complaints.all.version.count[,'Key'] <- factor(complaints.all.version.count[,'Key'], levels = unique(complaints.all.version.count[with(complaints.all.version.count, order(Record.y, Period, decreasing = TRUE)), 'Key']))
complaints.all.version.count[,'Period'] <- factor(complaints.all.version.count[,'Period'], levels = c('8 Weeks', '16 Weeks'), ordered = TRUE) 
...
chemistry.all.version.count[,'RecordedValue'] <- factor(chemistry.all.version.count[,'RecordedValue'], levels = unique(chemistry.all.version.count[with(chemistry.all.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
chemistry.all.version.count[,'Period'] <- factor(chemistry.all.version.count[,'Period'], levels = c('8 Weeks','16 Weeks'), ordered = TRUE)
...
chemistry.panel.version.count[,'RecordedValue'] <- factor(chemistry.panel.version.count[,'RecordedValue'], levels = unique(chemistry.panel.version.count[with(chemistry.panel.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
...
p.pouch.all <- ggplot(pouch.all.lims, aes(x=DateGroup, y=Rate, group=Key, color=Color)) + geom_line(color='black') + geom_point() + scale_color_manual(values = c('blue','red'), guide=FALSE) + geom_line(aes(y = UL), color='red', lty=2) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90)) + labs(title='Customer Pouch Complaints per Pouches Shipped', subtitle = 'Limit = +3 standard deviations', x='Date', y='4 Week Rolling Average') #+ annotate("text",x=x_positions,y=y_positions,label=pouch.annotations, size=4)
...
pouch.all.version.count[,'RecordedValue'] <- factor(pouch.all.version.count[,'RecordedValue'], levels = unique(pouch.all.version.count[with(pouch.all.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
pouch.all.version.count[,'Period'] <- factor(pouch.all.version.count[,'Period'], levels = c('8 Weeks','16 Weeks'), ordered = TRUE)
...
pouch.location.version.count[,'RecordedValue'] <- factor(pouch.location.version.count[,'RecordedValue'], levels = unique(pouch.location.version.count[with(pouch.location.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
...
instrument.all.3yr <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', subset(failures.df, Key=='Instrument'), c('Key'), threeyr.plot, 'Record', 'sum', 0)
...
instrument.all.version.count[,'RecordedValue'] <- factor(instrument.all.version.count[,'RecordedValue'], levels = unique(instrument.all.version.count[with(instrument.all.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
instrument.all.version.count[,'Period'] <- factor(instrument.all.version.count[,'Period'], levels = c('8 Weeks', '16 Weeks'), ordered=TRUE)
...
instrument.build.version.count[,'RecordedValue'] <- factor(instrument.build.version.count[,'RecordedValue'], levels = unique(instrument.build.version.count[with(instrument.build.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
...
soft.annotations <- c('')
y_pos.soft <- software.all.lims[(software.all.lims[,'DateGroup']) %in% x_pos.soft, 'Rate'] + 0.00005
p.software.all <- ggplot(software.all.lims, aes(x=DateGroup, y=Rate, group=Key, color=Color)) + geom_line(color='black') + geom_point() + scale_color_manual(values = c('blue','red'), guide=FALSE) + geom_line(aes(y = UL), color='red', lty=2) + scale_y_continuous(labels=percent) + scale_x_discrete(breaks=dateBreaks) + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90)) + labs(title='Customer Software Complaints per Pouches Shipped', subtitle = 'Limit = +3 standard deviations', x='Date', y='4 Week Rolling Average') + annotate("text",x=x_pos.soft,y=y_pos.soft,label=soft.annotations, size=4)
...
software.all.version.count[,'RecordedValue'] <- factor(software.all.version.count[,'RecordedValue'], levels = unique(software.all.version.count[with(software.all.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
software.all.version.count[,'Period'] <- factor(software.all.version.count[,'Period'], levels = c('8 Weeks', '16 Weeks'), ordered = TRUE)
...
software.build.version.count[,'RecordedValue'] <- factor(software.build.version.count[,'RecordedValue'], levels = unique(software.build.version.count[with(software.build.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
...
calendar.week.categories <- createCalendarLikeMicrosoft(year(Sys.Date())-3, 'Week')
correct.facets <- unique(aggregateAndFillDateGroupGaps(calendar.week.categories, 'Week', subset(failures.df, Key=='Accessory/Kitting'), c('RecordedValue'), plot.startDate.week, 'Record', 'sum', NA)$RecordedValue)
...
acckit.all.version.count[,'RecordedValue'] <- factor(acckit.all.version.count[,'RecordedValue'], levels = unique(acckit.all.version.count[with(acckit.all.version.count, order(Record.y, decreasing = TRUE)), 'RecordedValue']))
