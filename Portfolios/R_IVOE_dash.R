...
bladder.all.voe <- bladder.all.voe[with(bladder.all.voe, order(HoursBetweenBin, decreasing=TRUE)), ]
p.bladder.all.voe <- ggplot(subset(bladder.all.voe, RecordedValue != 'NoFailure')[with(subset(bladder.all.voe, RecordedValue != 'NoFailure'), order(HoursBetweenBin)), ], aes(x=DateGroup, y=Rate, fill=HoursBetweenBin)) + geom_bar(stat='identity') + scale_fill_manual(values = createPaletteOfVariableLength(bladder.all.voe, 'HoursBetweenBin'), name='') + scale_x_discrete(breaks=dateBreaks) + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90, hjust=1)) + labs(title='Effect of Enhanced Window Bladder QC:\nAll Failures/Lot Size in Field', y='Failures/Lot, Failure Count', x='Date of Lot Manufacture\n(Year-Week)') + facet_wrap(~Key, ncol=1, scale='free_y') + geom_text(aes(label=annotations.bladder, x=x_position, y=0), angle=90, hjust=-0.5, size=4)
...
board.thermo.total <- rbind(data.frame(DateGroup = board.thermo.total.rate$DateGroup, Type='(NCR Qty + RMA Failures)/Acutal Lot Size in Production Web', Key = board.thermo.total.rate$Key, Rate = board.thermo.total.rate$Rate),
...
board.image.total <- rbind(data.frame(DateGroup = board.image.total.rate$DateGroup, Type='(NCR Qty + RMA Failures)/Acutal Lot Size in Production Web', Key = board.image.total.rate$Key, Rate = board.image.total.rate$Rate),
...
board.camera.total <- rbind(data.frame(DateGroup = board.camera.total.rate$DateGroup, Type='(NCR Qty + RMA Failures)/Acutal Lot Size in Production Web', Key = board.camera.total.rate$Key, Rate = board.camera.total.rate$Rate),
...
p.board.camera.field <- ggplot(board.camera.field.rate, aes(x=DateGroup, y=Rate, fill='filler')) + geom_bar(stat='identity') + scale_fill_manual(values=createPaletteOfVariableLength(data.frame(Key='filler'),'Key'), guide=FALSE) + scale_x_discrete(breaks=dateBreaks) + scale_y_continuous(labels=percent) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(hjust=1, angle=90)) + labs(title='Camera Board Field Failures Per Lot Size', y='Failures/Lot', x='Date of Board Lot Manufacturing\n(Year-Week)')
p.board.camera.total <- ggplot(board.camera.total, aes(x=DateGroup, y=Rate, fill=Key)) + geom_bar(stat='identity') + facet_wrap(~Type, ncol=1, scale='free_y') + scale_fill_manual(values=createPaletteOfVariableLength(board.camera.total,'Key'), name='', labels=c('NCR','RMA')) + scale_x_discrete(breaks=dateBreaks) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(hjust=1, angle=90)) + labs(title='Camera Board Failures', x='Date of Manufacturing\n(Year-Week)', y='Failure Count, Failure Rate')
# valve board
...
board.valve.total <- rbind(data.frame(DateGroup = board.valve.total.rate$DateGroup, Type='(NCR Qty + RMA Failures)/Acutal Lot Size in Production Web', Key = board.valve.total.rate$Key, Rate = board.valve.total.rate$Rate),
...
p.board.valve.field <- ggplot(board.valve.field.rate, aes(x=DateGroup, y=Rate, fill='filler')) + geom_bar(stat='identity') + scale_fill_manual(values=createPaletteOfVariableLength(data.frame(Key='filler'),'Key'), guide=FALSE) + scale_x_discrete(breaks=dateBreaks) + scale_y_continuous(labels=percent) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(hjust=1, angle=90)) + labs(title='Valve Board Field Failures Per Lot Size', y='Failures/Lot', x='Date of Board Lot Manufacturing\n(Year-Week)')
p.board.valve.total <- ggplot(board.valve.total, aes(x=DateGroup, y=Rate, fill=Key)) + geom_bar(stat='identity') + facet_wrap(~Type, ncol=1, scale='free_y') + scale_fill_manual(values=createPaletteOfVariableLength(board.valve.total,'Key'), name='', labels=c('NCR','RMA')) + scale_x_discrete(breaks=dateBreaks) + theme(text=element_text(size=20, face='bold'), axis.text=element_text(size=20, face='bold', color='black'), axis.text.x=element_text(hjust=1, angle=90)) + labs(title='Valve Board Failures', x='Date of Manufacturing\n(Year-Week)', y='Failure Count, Failure Rate')
...
p.lids.voe <- ggplot(lids.rate, aes(x=DateGroup, y=Rate, group=Key, color=Key)) + scale_color_manual(values=pal.lids, name='') + geom_line() + geom_point() + scale_x_discrete(breaks=dateBreaks) + scale_y_continuous(labels=percent) + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90, hjust=1)) + labs(title='Lid Latch Failures per RMAs Shipped', x='Date\n(Year-Week)', y='Rolling 4-Week Average')
...
p.excitation.voe <- ggplot(excitation.ordered[with(excitation.ordered, order(Key)), ], aes(x=Lot, y=Record/LotSizeInField, fill=Key)) + geom_bar(stat='identity') + scale_fill_manual(values = pal.excitation, name='Hours Run', labels=c('0-100','100-500','500-1000','1000+','Unknown','')) + scale_y_continuous(labels=percent) + theme(text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=40, hjust=1)) + labs(title='LED Excitaiton Errors Reported by Bead Beater Lot Size in Field', x='Lot', y='7003 Complaints/Lot Size')
