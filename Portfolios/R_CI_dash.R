...
#3 Year view of pouches shipped
threeyr.start <- as.integer(year(Sys.Date()))-2
pouches.3yrview <- subset(with(pouches.3yr.df, aggregate(Record~Year+Version, FUN=sum)), Year >= threeyr.start)
p.PouchesShipped.3yr <- ggplot(pouches.3yrview, aes(x=Year, y=Record, fill=Version)) + geom_bar(stat='identity') + scale_fill_manual(values=createPaletteOfVariableLength(pouches.3yrview, 'Version'), name='Panel') + theme(axis.text=element_text(color='black',size=fontSize,face=fontFace), text=element_text(size=fontSize, face=fontFace)) + scale_y_continuous(labels=comma) + labs(title='Customer Pouches Shipped Per Year', x='Year', y='Pouches') + geom_text(aes(label=format(Record, big.mark = ',')), color='black', position=position_stack(vjust = 0.5), fontface="bold") + geom_label(data=with(pouches.3yrview, aggregate(Record~Year, FUN=sum)), inherit.aes = FALSE, aes(x=Year, y=Record, label=format(Record, big.mark=',')), fontface='bold', size=5, vjust=-0.5)
