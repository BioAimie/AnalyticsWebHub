...
bldg.515 <- subset(building.count, Building == '515')
p.515building <- ggplot(bldg.515, aes(x=DateGroup, y=Record, fill=Assay)) + geom_bar(stat="identity") + theme(plot.title=element_text(hjust=0.5), text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace , color='black'), axis.text.x=element_text(angle=90), legend.position="bottom") + labs(title='Environmental Contamination in 515 Building', x='Date\n(Year-Month)', y='Count') + scale_fill_manual(values=pal, name='') + guides(fill=guide_legend(nrow=7, by.row=TRUE))
bldg.515$Clean <- with(bldg.515, ifelse(Floor515 == 1, 'Clean', 'Neutral/Non-Clean'))
p.515building.clean <- ggplot(bldg.515, aes(x=DateGroup, y=Record, fill=Assay)) + geom_bar(stat="identity") + facet_wrap(~Clean) + theme(plot.title=element_text(hjust=0.5), text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace , color='black'), axis.text.x=element_text(angle=90), legend.position="bottom", plot.subtitle = element_text(hjust=0.5)) + labs(title='Environmental Contamination in 515 Building', subtitle = 'Clean Area (Floor 1) vs Other', x='Date\n(Year-Month)', y='Count') + scale_fill_manual(values=pal, name='') + guides(fill=guide_legend(nrow=7, by.row=TRUE))
