...
pal <- distinctColorPalette(length(assayNames))
...
p.cp.environmental <- ggplot(subset(cps.environ, !is.na(Cp)), aes(x=DateGroup, y=Cp)) + scale_y_continuous(breaks=pretty_breaks()) + geom_point(aes(color=Assay),size=3) + scale_color_manual(values=pal, name="") + theme(plot.title=element_text(hjust=0.5), text=element_text(size=fontSize, face=fontFace), axis.text=element_text(size=fontSize, face=fontFace, color='black'), axis.text.x=element_text(angle=90), legend.position = 'bottom') + labs(title='WEEKLY Environmental Contamination Cp', x='Date\n(Year-Week)', y='Cp') + guides(fill=guide_legend(nrow=5, by.row=TRUE)) + scale_x_discrete(breaks=dateBreaks)
