...
wherefound.count <- with(wherefound.fill[wherefound.fill[,'DateGroup'] >= findStartDate(calendar.df, 'Week', 8), ], aggregate(Record~RecordedValue, FUN=sum))
...
problemarea.lims <- addStatsToSparseHandledData(problemarea.rate, c('RecordedValue'), lagPeriods, TRUE, 3, 'upper', 0)
