...
calendar.week.trunc <- createCalendarLikeMicrosoft(year(Sys.Date())-2, 'Week')
...
instNCRs.ver.trunc <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', instNCRs.df, c('Version'), plot.startDate.week, 'Record', 'sum', NA)
instNCRs.ver.trunc$VersionGroup <- as.character(instNCRs.ver.trunc$Version)
instNCRs.ver.trunc$VersionGroup[as.character(instNCRs.ver.trunc$Version) == 'Torch Module' | as.character(instNCRs.ver.trunc$Version) == 'Torch Base'] <- 'Torch'
...
ncr.rate.ver.trunc <- mergeCalSparseFrames(instNCRs.ver.trunc, instBuilt.ver, c('DateGroup','VersionGroup'), c('DateGroup','Version'), 'Record', 'Record', NA, periods)
ncr.rate.ver.trunc$CRate <- ncr.rate.ver.trunc$Rate
ncr.rate.ver.trunc$CRate[as.character(ncr.rate.ver.trunc$DateGroup) %in% fix.dategroup & as.character(ncr.rate.ver.trunc$Version) == 'Torch Module'] <- NA
...
startPareto <- findStartDate(calendar.df, 'Week', 8, keepPeriods=0)
...
final.qc.ver <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', finalQC.df, c('Version','RecordedValue'), plot.startDate.week, 'Record', 'sum', 0)
...
wpfs.ver <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', wpfsNCR.df, c('Version','RecordedValue'), plot.startDate.week, 'Record', 'sum', 0)
...
dateBreaks.alt <- as.character(unique(calendar.week.trunc[calendar.week.trunc[,'DateGroup'] >= startDate.alt,'DateGroup']))[order(as.character(unique(calendar.week.trunc[calendar.week.trunc[,'DateGroup'] >= startDate.alt,'DateGroup'])))][seq(4,length(as.character(unique(calendar.week.trunc[calendar.week.trunc[,'DateGroup'] >= startDate.alt,'DateGroup']))), seqBreak)]
...
earlyfailures.batch.fill <- aggregateAndFillDateGroupGaps(calendar.df, 'Week', earlyfailures.batchsize, c('Version'), plot.startDate.week, 'BatchRate', 'mean', NA)
