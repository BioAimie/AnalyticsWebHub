...
makeSupplierbyPartChart <- function(ncrType, vendName, timeStart = NULL, timeEnd = NULL, partNumber = NULL) {
  ...
  # timeEnd <- '2016-05-16'
  # partNumber <- 'RFIT-FIT-0003'
  ##########################################
  if(is.null(vendName) || is.null(partNumber)) {
    return(0)
  }
  ...
  #make dategroups
  filteredData <- aggregateAndFillDateGroupGaps(calendar.week, 'Week', filteredData, 'PartNumber', startDate, 'Record', 'sum', 0)
  ...
  p.chart <- ggplot(subset(supp.part, as.character(supp.part$DateGroup) >= start & as.character(supp.part$DateGroup) <= end), aes(x=DateGroup, y=ARate, group=1)) + 
    geom_line() + geom_point() + xlab('Date\n(Year-Week)') + ylab('Part Acceptance Rate') + 
    theme(text=element_text(size=18), axis.text.x=element_text(angle=90, vjust=0.5,color='black',size=14),
    axis.text.y=element_text(hjust=1, color='black', size=14), legend.position='none') + ggtitle(paste('Acceptance Rate of Supplier Part\n',paste(partNumber,vendName, sep="   "))) + 
