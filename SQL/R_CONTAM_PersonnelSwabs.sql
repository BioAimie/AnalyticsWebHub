...
WHERE [RunStatus] LIKE 'Completed' AND [SampleId] LIKE '%1-KD%' AND [StartTime] >= GETDATE()-500
...
SELECT	
	...
	[Result],
	[Cp], 
	SUBSTRING([SampleId], 6, CHARINDEX('_', [SampleId], 6)-6) AS [Department], 
...
SELECT 
	...
	[Result],
	[Cp],
	UPPER(RTRIM(LTRIM([Department]))) AS [Department],
	[ConStatus],
	'Personnel' AS [Key],
	1 AS [Record],
	[DB]  
