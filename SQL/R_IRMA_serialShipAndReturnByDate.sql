...
WHERE [Tracker] = 'RMA' AND [Status] IN ('Closed','Accounting','Shipping','SalesOrderGeneration','Closure','InstrumentQCandDHR','ClosedFollowup')
...
INTO #properties
FROM [PMS1].[dbo].[vTrackers_AllPropertiesByStatus] WITH(NOLOCK)
WHERE [Tracker] = 'RMA' AND [PropertyName] IN ('RMA Title','RMA Type','Complaint Number','Hours Run','Service Completed')
...
WHERE (LEFT([SerialNo],2) IN ('FA','2F','HT','TM','KT') OR [Part Number] LIKE 'HTFA-%') AND [ServiceReq] = 1
