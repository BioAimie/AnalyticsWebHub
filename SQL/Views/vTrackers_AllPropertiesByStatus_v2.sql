USE [PMS1]
GO
IF OBJECT_ID('dbo.vTrackers_AllPropertiesByStatus','V') IS NOT NULL
DROP VIEW [dbo].[vTrackers_AllPropertiesByStatus]
USE [PMS1]
GO
CREATE VIEW [dbo].[vTrackers_AllPropertiesByStatus]
AS

WITH 
	[Right] ([Tracker], [TicketId], [TicketString], [Status], [CreatedBy], [CreatedDate], [InitialCloseDate], [Stage], [PropertyId], [PropertyName]) 
	AS
	(
		SELECT
			BT.[Tracker],
			...
			ON BT.[TicketId] = GT.[TicketId] INNER JOIN [RO_TRACKERS].[Trackers].[dbo].[TicketStages] TS WITH(NOLOCK)
				ON GT.[TicketId] = TS.[TicketId] INNER JOIN [RO_TRACKERS].[Trackers].[dbo].[StageTypes] ST WITH(NOLOCK)
					...
					(
						...
						ON TS.[TicketStageId] = GTH.[TicketStageId] INNER JOIN [RO_TRACKERS].[Trackers].[dbo].[TicketProperties] TKP WITH (NOLOCK) 
							ON GTH.[GeneralTicketHistoryId] = TKP.[GeneralTicketHistoryId] INNER JOIN [RO_TRACKERS].[Trackers].[dbo].[Properties] P WITH (NOLOCK)
								ON TKP.[PropertyId] = P.[PropertyId] INNER JOIN [RO_TRACKERS].[Trackers].[dbo].[TrackableProperties] TRP WITH (NOLOCK) 
												ON P.[TrackablePropertyId] = TRP.[TrackablePropertyId]
